...
접두 **없이** 주제만 적으면 조사 모드다 (DESIGN.md §10.8):

```
@tutor Kafka 컨슈머 그룹 리밸런싱
```

웹을 조사해 자료를 모으고 학습 리포트를 낸다. 문항은 자동으로 나오지 않는다 — 리포트를
읽고 *🎯 이해도 확인* 버튼을 눌렀을 때 그 자료로 10문항을 낸다. 자료는 repo가 아니라
회차에 매달리고 24시간 뒤 회수되므로, 같은 주제를 다시 물으면 **새로 조사한다**.

반복해서 볼 자료를 직접 넣고 싶으면 `docs/tutor-corpus.md`를 본다 — 자료를 파일로
두면 인용 대조·교차 검증·오답 노트가 그대로 살고, 코드 변경은 없다.
